...
from llambda.generator import CounterfactualGenerator, ResponseGenerator
from llambda.metrics.counterfactual import CounterfactualMetrics
from llambda.metrics.stereotype import StereotypeMetrics
from llambda.metrics.toxicity import ToxicityMetrics
...
class AutoEval:
    def __init__(
        ...
        """
        This class calculates all toxicity, stereotype, and counterfactual metrics support by LLaMBDA
    ...
    async def evaluate(
        ...
        for attribute in protected_words.keys():
            ...
            print(
                "LLaMBDA: Number of prompts containing {} words: {}".format(
                    attribute, protected_words[attribute]
                )
            )
            if protected_words[attribute] == 0:
                print(
                    "- LLaMBDA: The prompts satisfy fairness through unawareness for {} words, the recommended risk assessment only include Toxicity".format(
                        attribute
                    )
                )
            else:
                print(
                    "- LLaMBDA: The prompts do not satisfy fairness through unawareness for {} words, the recommended risk assessments include Toxicity, Stereotype, and Counterfactual Discrimination.".format(
                        attribute
                    )
                )
        ...
        if total_protected_words > 0:
            ...
            print("---------------------------------------")
            print("LLaMBDA: Evaluating metrics...")
            cf_results = {}
        ...
        if metrics is None:
            ...
            for metric in metrics:
                if metric in DefaultMetrics:
                    ...
                    raise RuntimeError(
                        "LLaMBDA: If `metrics` is a list, it should be a subset of following list ['counterfactual', 'stereotype', 'toxicity']"
                    )
            metrics = tmp
        elif isinstance(metrics, dict):
            for key in metrics.keys():
                if key not in DefaultMetrics.keys():
                    raise KeyError("LLaMBDA: {} not found".format(key))
                self._check_list(
                    metrics[key], DefaultMetrics[key], "metrics['" + key + "']"
                )
        else:
            raise TypeError(
                "LLaMBDA: Attribute `metrics` should be a list of strings or a dictionary of list of strings"
            )
        return metrics
    ...
    def _validate_list_type(input_variable: List[str]) -> List[str]:
        ...
        if isinstance(input_variable, list):
            if len(input_variable) == 0 or not isinstance(input_variable[0], str):
                raise RuntimeError(
                    "LLaMBDA: List {} should contain strings and can't be empty.".format(
                        input_variable
                    )
                )
    ...
    def _check_list(list1: List[str], list2: List[str], error_tag: Any) -> None:
        ...
        for list1_i in list1:
            if not isinstance(list1_i, str):
                raise TypeError(
                    "LLaMBDA: Type of list '{}' should be a string.".format(error_tag)
                )
            elif list1_i not in list2:
                raise RuntimeError(
                    "LLaMBDA: Provided '{}' metric is not an in-built LLaMBDA metric".format(
                        error_tag
                    )
                )
