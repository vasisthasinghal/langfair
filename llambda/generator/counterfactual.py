...
from llambda.constants.word_lists import (
    FEMALE_WORDS,
...
from llambda.generator.generator import ResponseGenerator
...
class CounterfactualGenerator(ResponseGenerator):
    ...
    def parse_texts(
        ...
        LLaMBDA: Either custom_list or attribute must be None.
        """
        assert custom_list or attribute in ["race", "gender"], """
        LLaMBDA: If custom_list is None, attribute must be 'race' or 'gender'.
        """
        result = []
        for text in texts:
            result.append(
    ...
    def create_prompts(
        ...
        LLaMBDA: Either custom_dict or attribute must be None.
        """
        assert custom_dict or attribute in [
            "gender",
            "race",
        ], "LLaMBDA: If custom_dict is None, attribute must be 'gender' or 'race'."
    ...
    async def generate_responses(
        ...
        if self.llm.temperature == 0:
            assert (
                count == 1
            ), "LLaMBDA: temperature must be greater than 0 if count > 1"
        ...
        print("LLaMBDA: Responses successfully generated!")
        return {
            "data": {
    ...
    def _subset_prompts(
        ...
        LLaMBDA: Provided prompts do not contain any {attribute_to_print} words.
        """
        print(
            f"LLaMBDA: {attribute_to_print} words found in {len(prompts_subset)} prompts."
        )
