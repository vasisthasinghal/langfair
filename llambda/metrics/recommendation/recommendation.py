...
from llambda.metrics.recommendation import metrics
from llambda.metrics.recommendation.metrics.baseclass.metrics import Metric
...
class RecommendationMetrics:
    ...
    def evaluate_pairwise(
        ...
        ), "LLaMBDA: The same number of recommendation lists must be provided for both groups"
        K = len(rec_lists1[0])
        for i in range(len(rec_lists1)):
            assert (
                len(rec_lists1[i]) == K
            ), "LLaMBDA: Recommendation lists must all be of equal length"
            assert (
                len(rec_lists2[i]) == K
            ), "LLaMBDA: Recommendation lists must all be of equal length"
        return {
