...
class CooccurrenceBiasMetric:
    def __init__(
        ...
        how : str, default='mean'
            If defined as 'mean', evaluate method returns average COBS score, else dictinary with COBS(w) for each word 'w'.
        """
        ...
        if stereotype_word_list is not None:
            assert isinstance(stereotype_word_list, list) and all(
                ...
                LLaMBDA: If provided, `stereotype_word_list` must be list of strings
            """
        ...
        if demographic_group_word_lists is None:
            ...
            assert len(demographic_group_word_lists.keys()) == 2, """
                `LLaMBDA: demographic_group_word_lists` must be a dictionary with exactly two keys
            """
    ...
    def evaluate(self, responses: List[str]) -> Union[float, Dict[str, float]]:
        ...
        if len(cobs_scores_list) == 0:
            print(
                "LLaMBDA: None of the target words co-occur with both lists of attribute words. Unable to calculate COBS score."
            )
            return None
    ...
    def _prep_lists(
        ...
        if not (
            ...
            print(
                "LLaMBDA: The provided sentences do not contain words from both word lists. Unable to calculate Co-occurrence bias score."
            )
